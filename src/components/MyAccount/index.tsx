  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error("Failed to change password: " + res.message);
      }
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteAddress = async (id: number) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error("Failed to delete address: " + res.message);
      }
    } catch (err) {
      ...
      <section className="overflow-hidden py-20 bg-gray-2">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
              ...
              <div className="flex xl:flex-col">
                <div className="hidden lg:flex flex-wrap items-center gap-5 py-6 px-4 sm:px-7.5 xl:px-9 border-r xl:border-r-0 xl:border-b border-gray-3">
                  ...
                  <div>
                    ...
                    <p className="text-custom-xs">Member Since {user?.created_at ? new Date(user.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : 'Unknown'}</p>
                  </div>
                </div>
          ...
          <!--== user dashboard content start ==--> */}
            ...
            >
              <p className="text-dark">
                Hello {user?.name || "User"} (not {user?.name || "User"}?{" "}
                <button
                  onClick={() => {
                    dispatch(logout());
                    window.location.href = "/";
          ...
          <!-- addresses tab content start --> */}
            <div
              className={`w-full flex-col sm:flex-row gap-7.5 ${
                activeTab === "addresses" ? "flex" : "hidden"
              }`}
            >
              <div className="xl:max-w-[500px] w-full bg-white shadow-1 rounded-xl">
                ...
                <div className="p-4 sm:p-7.5">
                  {isLoadingAddresses ? (
                    <p className="text-custom-sm text-dark-4">Loading shipping addresses...</p>
                  ) : shippingAddresses.length > 0 ? (
                    <div className="flex flex-col gap-6">
                      {shippingAddresses.map((address) => (
                        <div key={address.id} className="flex flex-col gap-4 border-b border-gray-3 pb-4 last:border-0 last:pb-0">
                          <div className="flex justify-between items-start">
                            {address.is_default && <p className="text-xs font-bold text-blue uppercase mb-2">Default Address</p>}
                            <button
                              onClick={() => handleDeleteAddress(address.id)}
                              className="text-red hover:text-red-dark transition-colors duration-200"
                          ...
                          <p className="flex items-center gap-2.5 text-custom-sm">
                            ...
                            <span className="font-medium">{address.label || address.company_name || address.contact_name || user?.name}</span>
                          </p>
                    ...
                    <p className="text-custom-sm text-dark-4">No shipping addresses found.</p>
                  )}
                </div>
              </div>
            </div>
